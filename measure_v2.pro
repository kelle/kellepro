pro measure, input_file, file_key=file_key
...
root='/data/hillary/2/kelle/optical_spectra/2M_all/fits/'

READCOL, input_file,ref,desig,sp_type,spectra_files,format='A,A,A,A',/PRESERVE_NULL

num_files = n_elements(spectra_files)
output_file=input_file+'_indices.txt'

OPENW, unit, output_file,/get_lun ; open output file for writing
...
name[i]='CaOH'
num = avgflux(6230., 6240., wavelength, flux)
den = avgflux(6345., 6354., wavelength, flux)
index[i] = strtrim(string(num/den),1)
print, name[i],index[i] & i=++i
...
save, file=output_file+'.sav',indices,index_name
