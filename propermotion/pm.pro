...
;       v9: postscript output for each trial
;      v10: several bug fixes
;           - divided ra_offset by cos(dec) so predicted position crosses display in
;             correct position
;           - changed way crosses are displayed (one_ray instead of
;             plots)
;           - previous refdata.dat are identified and new ref_data.dat
;             files are written
;        corrected way uncertainty on mu_total is
;        calculated. (total_mu_sig on line 511.).
;-

PRO pm;, coords_file, POSS_image_file, image_file_2M, tvmag=tvmag, log=log_set
...
IF file_test(logfile) THEN BEGIN ;previous logfile exists
  ...
  CLOSE, lun_log
  FREE_LUN, lun_log
  run=strn((n_elements(WHERE(STRCMP(logsarr,'*',1) eq 1))/5)+1)
ENDIF ELSE BEGIN
  old_file=0
  run=strn(1)
ENDELSE
...
IF GET_FILE(obj,'logs/',/SILENT,/COUNT) GE 1 THEN BEGIN ; previous ref data file exists
 ...
 READ, answer, PROMPT='Do you want to use this data instead of choosing new reference stars? (y/n): '
	IF answer eq 'y' THEN BEGIN
	  ref_data_file=GET_FILE(obj,'logs/')
	  restore, ref_data_file
	  choose=0  
	ENDIF ELSE IF answer eq 'n' THEN BEGIN
	  choose=1
	  ref_data_file='logs/'+obj+'_refdata_'+run+'.dat'
	ENDIF ELSE BEGIN
...
;-----------------
;NEED TO MAKE SURE DATE ENTERED HERE (date_xy) IS CORRECT BEFORE
;PROCEEDING
 WHILE date_xy[0] LT 1900 OR date_xy[0] GT 2100 OR date_xy[1] gt 12 OR date_xy[2] gt 31 DO BEGIN 
  PRINT, strn(date_xy) + ' : Incorrect date format or unlikely date (year <1900 or >2100), try again.'
  READ, date_xy, PROMPT='Enter date of xy coords for PM target as listed above (yyyy,mm,dd): '
 ENDWHILE 
;if date_xy[1] gt 12 OR date_xy[2] gt 31 THEN problem
...
WHILE (quit_ref eq 0) AND (refn lt n_ref_lines) DO BEGIN 
  ...
  tvscl, rebin(image,nx*tvmag,ny*tvmag,/s),top=top
   ...
   ;mark previously chosen ref stars
    FOR m=0,i do begin
...
;coeffs1=coeffs(ra_target, dec_target ,x_target, y_target, ra_ref, dec_ref, x_ref, y_ref, obj, run, log=log_set)
coeffs1=coeffs(ra_ref, dec_ref, x_ref, y_ref, obj, run, log=log_set)
...
;SOLVE FOR RA,DEC COORDS OF REFERENCE STARS IN SECOND EPOCH COORD SYSTEM BUT AT FIRST
;EPOCH POSITION
...
;solve for platescale using evalues of transformation matrix
hes=elmhes(trans,/double)
evalues=float(hqr(hes))
ra_platescale=abs(evalues[0])
dec_platescale=abs(evalues[1])

;PRINT SOLVED PLATESCALE of 2nd EPOCH IMAGE
print, 'platescales:'
print, ra_platescale
print, dec_platescale

;SOLVE FOR COORDS OF TARGET at FIRST EPOCH POSITION (POSS)
...
print, ra_target_poss, dec_target_poss
...
IF keyword_set(log_set) and choose eq 1 THEN BEGIN
 im_file='logs/'+obj+'_solution_'+run+'.ps'
...
IF see eq 'y' THEN BEGIN
 coeffs2=coeffs(ra_ref, dec_ref, x_ref, y_ref, obj, run, log=0, view=1)
ENDIF
...
mu_ra_sig=ra_sig/abs(year_diff)*60*60
mu_dec_sig=dec_sig/abs(year_diff)*60*60

Print, 'pm in arcsec per year'
print, mu_ra, mu_dec
print, mu_ra_sig, mu_dec_sig

total_mu=sqrt(delta_ra^2 + delta_dec^2)/abs(year_diff)*60*60
;total_mu_sig=sqrt(mu_ra_sig^2*abs(mu_ra)/total_mu + mu_dec_sig^2*abs(mu_dec)/total_mu)
;CORRECTED AFTER SCHMIDT07 PUBLISHED
total_mu_sig=sqrt((mu_ra_sig*mu_ra/total_mu)^2 + (mu_dec_sig*mu_dec/total_mu)^2)
...
if mu_ra lt 0 then slen_ra = 7 else slen_ra = 6
mu_ra_s=strmid(strn(round(mu_ra*10000)/10000.),0,slen_ra)

if mu_dec lt 0 then slen_dec = 7 else slen_dec = 6
mu_dec_s=strmid(strn(round(mu_dec*10000)/10000.),0,slen_dec)

mu_ra_sig_s=strmid(strn(round(mu_ra_sig*10000)/10000.),0,6)
mu_dec_sig_s=strmid(strn(round(mu_dec_sig*10000)/10000.),0,6)

total_mu_s=strmid(strn(round(total_mu*10000)/10000.),0,6)
total_mu_sig_s=strmid(strn(round(total_mu_sig*10000)/10000.),0,6)
...
IF abs(year_diff) lt 1. then print, 'WARNING: BASELINE VERY SHORT'
...
;CONVERT PREDICTED SPITZER POSITION TO STRING FOR LOG
...
one_ray, (x50*zoom)-tk,(y50*zoom), 2*tk, 0, color=purple, thick=2
one_ray, (x50*zoom),(y50*zoom)-tk, 2*tk, 90, color=purple, thick=2
...
one_ray, (x80*zoom)-tk,(y80*zoom), 2*tk, 0, color=yellow, thick=2
one_ray, (x80*zoom),(y80*zoom)-tk, 2*tk, 90, color=yellow, thick=2
...
one_ray, (x1*zoom)-tk,(y1*zoom), 2*tk, 0, color=black, thick=2
one_ray, (x1*zoom),(y1*zoom)-tk, 2*tk, 90, color=black, thick=2
...
one_ray, (x2*zoom)-tk,(y2*zoom), 2*tk, 0, color=blue, thick=2
one_ray, (x2*zoom),(y2*zoom)-tk, 2*tk, 90, color=blue, thick=2
...
one_ray, (xs*zoom)-tk,(ys*zoom), 2*tk, 0, color=red, thick=2
one_ray, (xs*zoom),(ys*zoom)-tk, 2*tk, 90, color=red, thick=2
...
IF keyword_set(log_set) THEN BEGIN
 IF choose eq 1 then begin
 aspectRatio=float(!D.Y_Vsize)/!D.X_Vsize
 pos_im_file='logs/'+obj+'_positions_'+run+'.ps'
 out_im=tvrd(TRUE=1) ; get image from grapics window
 set_plot, 'ps'
...
;readin just modified log
 ...
 data_log=WHERE(STRCMP(lines_log,'*',1) eq 1)
 ...
 comment=strtrim(strmid(log_result[4],strpos(log_result[4],":")+1,30),0)
