...
PRO pm_v2;, coords_file, POSS_image_file, image_file_2M, tvmag=tvmag, log=log_set
program='pm_v2'
...
dist=line[1]
if dist lt 1.5 then print, 'WARNING: Make sure first object in ref_lis is proper motion target'
ra_target = line[6]
dec_target = line[7]
j_mag = line[9]
date_target=line[11]
...
plots, (x2)*zoom+tk, (y2)*zoom-tk, /device
plots, (x2)*zoom-tk, (y2)*zoom+tk, /device,/continue, color=blue,thick=2
xyouts, (x2)*zoom+xtk,(y2)*zoom+ytk,'2MASS',/DEVICE, charsize=2, color=red, charthick=2, orient=txt_rot
...
plots, (xs)*zoom+tk, (ys)*zoom-tk, /device
plots, (xs)*zoom-tk, (ys)*zoom+tk, /device,/continue, color=red,thick=2
xyouts, (xs)*zoom+xtk+(-1*s_mura*xtk),(ys)*zoom+ytk+(-1*s_mudec*ytk),'2005',/DEVICE, charsize=2, color=red, charthick=2, orient=txt_rot
...
IF keyword_set(log_set) THEN BEGIN
 ...
 if dist lt 1.5 then printf, lun_rlog, 'WARNING: Make sure first object in ref_lis is proper motion target'
 printf,lun_rlog, ' '
